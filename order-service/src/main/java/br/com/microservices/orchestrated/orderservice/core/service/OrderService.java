...
public class OrderService {
    ...
    /**
     * Save transaction mongoDb instance
     * send payload event to producer
     * @param orderRequest
     * @return
     */
    public Order createOrder(OrderRequest orderRequest) {
        Order order = Order
         ...
         producer.sendEvent(jsonUtil.toJson(createPayloadEvent(order)));

         return order;
    }

    private Event createPayloadEvent(Order order) {
        Event event = Event
                .builder()
                .orderId(order.getId())
